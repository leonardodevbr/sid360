...
export async function downloadInstallmentRecibo(installmentId, filename) {
  const response = await api.get(`/installments/${installmentId}/recibo`, { responseType: 'blob' });
  const resolvedFilename = filename
    ?? filenameFromContentDisposition(response.headers)
    ?? `recibo-parcela-${installmentId}.pdf`;
  const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
  const link = document.createElement('a');
  link.href = url;
  link.download = resolvedFilename;
