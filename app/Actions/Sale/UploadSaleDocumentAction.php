...
class UploadSaleDocumentAction
{
     ...
     */
    public function execute(Sale $sale, UploadedFile $file, string $type, ?int $uploadedBy = null): SaleDocument
    {
        $sale->loadMissing('lot.development');
        $developmentId = $sale->lot?->development?->id ?? 'sem-empreendimento';

        $extension = strtolower($file->getClientOriginalExtension() ?: $file->extension() ?: 'bin');
        $path = "developments/{$developmentId}/sales/{$sale->id}/documents/{$type}/".Str::uuid().".{$extension}";
