        ...
        return [
            ...
            'sales' => $sales->map(function (Sale $sale) use ($today): array {
                $installments = $sale->installments->map(
                    fn (Installment $installment): array => PortalInstallmentMapper::toArray($installment, $sale, $today),
                )->values();
                ...
                return [
                    'id' => $sale->id,
                    'contract_no' => PortalInstallmentMapper::contractNumber($sale),
                    'sale_date' => $sale->sale_date?->toDateString(),
                    'status' => $sale->status,
                    'total_value' => (int) $sale->total_value,
                    'down_payment' => (int) $sale->down_payment,
                    'installments_count' => $sale->installments_count,
                    'efi_carnet_id' => $sale->efi_carnet_id,
                    'efi_carnet_pdf' => $sale->efi_carnet_pdf,
                    'efi_carnet_link' => $sale->efi_carnet_link,
                    'lot' => [
